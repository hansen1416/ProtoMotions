...
trunk), and a small trainable MLP adds a per-env residual correction:
    ...
    delta    = adapter_mlp(obs)      # small trainable MLP, TRAINABLE
    output   = base_out + delta

`adapter_mlp` reads the exact same (normalized, concatenated) input the frozen trunk sees --
`morphology_obs` is already one of the trunk's `in_keys`, so it's already part of that input,
same as the original "just concatenate beta into obs" approach that worked pre-freeze. The
only difference is *which* weights get to learn from it: previously the whole network did,
now only this small head does, since the trunk stays frozen.
