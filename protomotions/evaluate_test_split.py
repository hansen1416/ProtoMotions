...
#
"""Evaluate a checkpoint once on the frozen test split.

Test clips are streamed in bounded clip batches. Each per-clip file contains all body
...
``eval_test/*`` protocol without materializing the complete validation corpus on one GPU.
...
This is a one-way test gate: the test manifest is loaded only by this standalone evaluator and
is never exposed by GlobalClipPool during training.

Usage:
    python protomotions/evaluate_test_split.py \
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Stream a checkpoint over the frozen test split",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Motion download cache (default: <checkpoint dir>/test_motion_cache)",
    )
    parser.add_argument(
        "--keep-downloaded-clips",
        action="store_true",
        help="Keep processed test .pt files instead of bounding disk use per batch",
    )
    parser.add_argument(
        "--limit-clips",
        type=int,
        default=None,
        help="Evaluate only the first N test clips (smoke testing only)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Summary JSON (default: <checkpoint dir>/test_eval_report.json)",
    )
    parser.add_argument(
        "--details-output",
        type=str,
...
def _aggregate_records(records: Sequence[Dict[str, Any]], evaluator_config) -> Dict[str, Any]:
    ...
    metrics: Dict[str, Any] = {
        "eval_test/num_motions": len(records),
        "eval_test/success_rate": 1.0
    ...
    for name in evaluator_config.evaluation_components:
        ...
        if component_config.static_params.get("threshold") is not None:
            metrics[f"eval_test/{name}/failure_rate"] = sum(
                row["failed"] for row in component_rows
            ) / len(component_rows)
        metrics[f"eval_test/{name}/mean"] = sum(
            row["mean"] for row in component_rows
        ) / len(component_rows)
        metrics[f"eval_test/{name}/max"] = max(
            row["max"] for row in component_rows
        )
        metrics[f"eval_test/{name}/min"] = min(
            row["min"] for row in component_rows
        )

    additional_fields = {
        "normalized_jerk": "eval_test/normalized_jerk_mean",
        "high_jerk_frame_percentage": "eval_test/high_jerk_frame_percentage_mean",
        "action_delta_mean_rad": "eval_test/action_delta_mean_rad",
        "action_delta_max_rad": "eval_test/action_delta_max_rad",
        "action_rate_mean_rad_s": "eval_test/action_rate_mean_rad_s",
        "action_delta_mean_deg": "eval_test/action_delta_mean_deg",
        "action_delta_max_deg": "eval_test/action_delta_max_deg",
    }
    ...
    for source_id, source_name in MOTION_SOURCE_NAMES.items():
        ...
        prefix = f"eval_test_{source_name}"
    ...
    if known_source_records:
        metrics["eval_test/source_conditioned_success_rate"] = (
...
def _evaluate_clip_batch(agent, motion_lib, clip_ids, batch_index):
    ...
    for key, value in chunk_log.items():
        if key.startswith("eval/"):
            key = key.replace("eval/", "eval_test/", 1)
        elif key.startswith("eval_"):
            key = key.replace("eval_", "eval_test_", 1)
...
def main():
    ...
    if "global_clip_pool" not in motion_lib_config._target_:
        raise RuntimeError(
            ...
            "GlobalClipPool -- there is no frozen split configuration to evaluate."
        )
    ...
    # Standalone test evaluation does not need a meaningful training resident pool. Keeping one clip
    ...
    cache_dir = (
        ...
        else checkpoint.parent / "test_motion_cache"
    )
    ...
    if not motion_lib.has_eval_holdout():
        raise RuntimeError(
            "GlobalClipPool reports no validation holdout needed to initialize the test evaluator."
        )

    # GlobalClipPool deliberately never downloads the test manifest during training. This
    # standalone gate resolves and validates it only after the checkpoint has been loaded.
    split_metadata = motion_lib._load_split_metadata(
        motion_lib.config.r2_source, motion_lib.config.split_metadata_name
    )
    test_manifest_name = motion_lib._split_role_remote_path(split_metadata, "test")
    test_remote_name, test_manifest_sha256 = motion_lib._load_manifest(
        motion_lib.config.r2_source, test_manifest_name
    )
    test_spec = split_metadata["manifests"]["test"]
    if test_manifest_sha256 != str(test_spec["sha256"]):
        raise RuntimeError(
            "Test manifest SHA-256 does not match split metadata: "
            f"actual={test_manifest_sha256}, expected={test_spec['sha256']}"
        )
    if len(test_remote_name) != int(test_spec["clip_count"]):
        raise RuntimeError(
            "Test manifest clip count does not match split metadata: "
            f"actual={len(test_remote_name)}, expected={test_spec['clip_count']}"
        )
    overlap = set(test_remote_name) & (set(motion_lib.rank_clip_ids) | set(motion_lib.eval_holdout_clip_ids))
    if overlap:
        raise RuntimeError(
            f"Test manifest overlaps train/validation clips ({len(overlap)}), "
            f"including {sorted(overlap)[:5]}"
        )
    # Reuse the validated holdout loader only after replacing its private map in this
    # standalone process; training code has no test-manifest path or loader call.
    motion_lib._eval_holdout_remote_name = dict(test_remote_name)
    motion_lib.eval_holdout_clip_ids = list(test_remote_name)

    test_clip_ids = list(motion_lib.eval_holdout_clip_ids)
    if args.limit_clips is not None:
        test_clip_ids = test_clip_ids[: args.limit_clips]
    num_batches = math.ceil(len(test_clip_ids) / args.clip_batch_size)
    shape_panel_index = agent.evaluator._eval_shape_panel_index()

    output_path = (
        Path(args.output).resolve()
        if args.output is not None
        else checkpoint.parent / "test_eval_report.json"
    )
    details_path = (
    ...
    log.info(
        f"Evaluating epoch {agent.current_epoch} on {len(test_clip_ids)} test "
    ...
    def current_report(complete: bool) -> Dict[str, Any]:
        return {
            ...
            "test_manifest_clip_count": int(test_spec["clip_count"]),
            "shape_panel_index": int(shape_panel_index),
            "num_test_clips_expected": len(test_clip_ids),
            "num_test_clips_processed": len(all_records),
    ...
    try:
        with open(details_path, "w") as details_stream:
            for batch_index, start in enumerate(
                range(0, len(test_clip_ids), args.clip_batch_size)
            ):
                clip_ids = test_clip_ids[start : start + args.clip_batch_size]
                log.info(
                    f"Test clip batch {batch_index + 1}/{num_batches}: "
                ...
                observed_success = evaluator_chunk_metrics.get(
                    "eval_test/success_rate"
                )
                recorded_success = batch_metrics["eval_test/success_rate"]
    ...
    print("\n" + "=" * 68)
    print(
        f"TEST SPLIT EVALUATION ({motion_lib.config.split_version}, "
    ...
    log.info(f"Wrote complete test report to {output_path}")
