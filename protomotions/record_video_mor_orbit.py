...
with a slow orbiting camera instead of the fixed diagonal view.

Fork of record_video_mor.py (kept unmodified) -- same functionality plus
--camera-orbit. See that file's docstring for the base usage patterns; this one
...
With --camera-orbit, the camera sweeps around the centroid of all envs over the
full recording instead of staying fixed, completing --camera-orbit-revolutions
full 360-degree turns by the last frame. This is independent of the underlying
motion's own length: with --same-motion, the extracted temp motion file only
ever contains shape-variants of the one selected clip (see
extract_motion_across_shapes), so each env has nothing else to resample into on
reset -- looping --video-steps past one playthrough just replays the same clip
while the camera keeps sweeping, safe by construction.

Example
-------
    # Slow full orbit around all 128 body shapes performing the same clip,
    # looping ~3.6x while the camera completes one 360-degree turn over 24s:
    python protomotions/record_video_mor_orbit.py \\
        ...
        --simulator isaacgym \\
        --motion-index 5248 --num-envs 128 --same-motion \\
        --compact-spawn-spacing 2.0 --fps 30 --video-steps 720 \\
        --camera-orbit --camera-orbit-revolutions 1.0 \\
        --output output/videos/orbit_M001062.mp4 \\
...
def create_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Slowly orbit the camera around the env grid over the full recording, "
            "instead of the default fixed diagonal view. The underlying motion loops "
            "independently (safe with --same-motion, see extract_motion_across_shapes) "
            "if --video-steps exceeds one playthrough, so a slow multi-revolution orbit "
            "over a short clip just means the clip repeats a few times."
        ),
    )
    parser.add_argument(
        "--camera-orbit-revolutions",
        type=float,
        default=1.0,
    ...
    parser.add_argument(
        "--camera-orbit-elevation-scale",
        type=float,
        default=0.5,
        help=(
            "Camera height above the grid centroid, as a multiple of the env-grid "
            "spread (was hardcoded to 0.5x spread). Lower = lower camera."
        ),
    )
    parser.add_argument(
        "--show-markers",
...
def _setup_orbit_camera(
    simulator, env, num_steps, revolutions=1.0, distance_scale=1.0, elevation_scale=0.5,
):
    ...
    height = float(centroid[2]) + spread * elevation_scale
    target = gymapi.Vec3(
    ...
    def _update():
        angle = 2.0 * math.pi * revolutions * (frame_counter["i"] / denom)
        cam_pos = gymapi.Vec3(
            float(centroid[0]) + radius * math.cos(angle),
            float(centroid[1]) + radius * math.sin(angle),
            height,
        )
        gym.viewer_camera_look_at(viewer, None, cam_pos, target)
        frame_counter["i"] += 1

    # Note: don't call _update() here -- render() (called once per recorded frame in
    # the recording loop, plus once more during the unrecorded warm-up render before
    # this setup runs) already drives it exactly num_steps times across the actual
    # recorded frames; an extra manual call here would shift every frame's angle by
    # one step and overshoot the final revolution count.
    simulator._update_camera = _update
...
def record_video(
    ...
    camera_orbit_elevation_scale=0.5, show_markers=True,
):
    ...
    if camera_orbit:
        _setup_orbit_camera(
            ...
            elevation_scale=camera_orbit_elevation_scale,
        )
    else:
...
def main():
    ...
    if args.simulator != "isaacgym":
        raise SystemExit(
            f"record_video_mor_orbit.py only supports --simulator isaacgym "
    ...
    try:
        record_video(
            ...
            camera_orbit_elevation_scale=args.camera_orbit_elevation_scale,
