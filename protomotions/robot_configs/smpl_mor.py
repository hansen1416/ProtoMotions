...
@dataclass
class SmplMorRobotConfig(RobotConfig):
    # these are important bodies for tracking / observation / marker visualization.
    ...
    # contact from these bodies should not immediately trigger failure. These are the normal foot contact bodies. Other unexpected body contacts may be treated as falling / failure.
    ...
    # when the humanoid is reset to a default standing pose, fallback scalar for multiple shape.
    ...
    control: ControlConfig = field(
        default_factory=lambda: ControlConfig(
            ...
            # a fixed effort_limit. Scale each DOF's gains by its own body's actual mass (not one
            # whole-actor scalar -- limb length/mass ratio varies substantially independent of
            # total mass across these 128 shapes, e.g. thigh/shin length vs total_mass r^2 is only
            # 0.05-0.21, so a global scalar leaves most per-limb mismatch uncorrected).
