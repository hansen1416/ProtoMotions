...
class MotionVisualizerSmoothness:
    def __init__(
        ...
        if not (robot_name == "smpl_mor"
            ...
            print("Visualize morphology only")
            exit()
        ...
        print(
            f"Loaded morphology MotionLib with {self.num_envs} motions/envs"
        )
        print(
            f"Requested morphology assets: {self.requested_morphology_asset_ids[:8]}"
        )
        # morphology =======
        ...
        if self.requested_morphology_asset_ids is not None:
            extra_params["morphology_asset_ids"] = self.requested_morphology_asset_ids
        # morphology =======
    ...
    def _translate_morphology_motions_to_grid(self):
        ...
        for env_id, motion_id in enumerate(self.env_motion_ids.detach().cpu().tolist()):
            ...
            self.motion_lib.gts[start:end, :, :2] += delta_xy.view(1, 1, 2)
    ...
    def _precompute_motion_smoothness(self):
        ...
        for frame_idx in range(self.current_motion_length):
            ...
            all_positions.append(state.rigid_body_pos)

            if state.rigid_body_vel is not None:
                all_velocities.append(state.rigid_body_vel)
            else:
                all_velocities.append(torch.zeros_like(state.rigid_body_pos))
        ...
        for frame_idx in range(T):
            ...
            pos_window = positions_tensor[window_start:window_end]
            vel_window = velocities_tensor[window_start:window_end]
