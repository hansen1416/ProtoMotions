...
#
"""
Mimic Environment — Residual PD Action Control
===============================================

Same as mlp.py but uses residual PD control:
    q_target = q_ref(t) + residual_scale * tanh(action)

When action=0 the controller tracks the reference pose exactly. The policy only
needs to output small corrections for dynamics and contacts, eliminating the large
sustained actions required by standard PD for non-default poses (crawl, kneel, squat).

Intended use: fine-tune from a converged mlp.py checkpoint.
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    from protomotions.envs.action import make_residual_pd_action_config
    ...
    return EnvConfig(
        ...
        action_config=make_residual_pd_action_config(robot_cfg, residual_scale=0.3),
