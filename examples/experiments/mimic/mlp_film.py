...
#
"""
Mimic + FiLM Environment Configuration
=======================================

Identical to mimic/mlp.py except that the actor and critic use FiLM-conditioned
MLPs (FiLMMLPWithCond) instead of the flat-concat MLPs (MLPWithConcat).

Difference vs. mimic/mlp.py
----------------------------
- morphology_obs is NO LONGER concatenated into the main trunk input.
- Instead it is passed to a separate conditioner network that produces
  per-layer FiLM parameters (gamma, beta) which modulate the trunk.
- All other env / reward / termination settings are identical.

This lets you do an apples-to-apples comparison:
    mimic/mlp.py       → baseline: concat morphology into flat obs
...
def agent_config(
    ...
    """Build agent configuration with FiLM-conditioned actor and critic."""
    from protomotions.agents.common.film_mlp import FiLMMLPConfig
    ...
    actor_config = PPOActorConfig(
        ...
        mu_model=FiLMMLPConfig(
            ...
            cond_hidden_units=[64, 64],
            cond_activation="silu",
            beta_norm_scale=3.0,
        ),
    )

    critic_config = FiLMMLPConfig(
        in_keys=_MAIN_OBS_KEYS,
        cond_keys=_COND_KEYS,
        normalize_obs=True,
        norm_clamp_value=5,
