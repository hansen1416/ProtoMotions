...
#
"""
Mimic Environment — GMT-style MoE Actor + Key-Joint Position Reward Weighting
===============================================================================

Same as mlp_moe.py in every respect except the `gt_rew` (position tracking)
reward weights root/wrists/ankles more heavily than the rest of the body.

Motivated by a per-body-joint error breakdown on hhi_moe_20946_2shape's worst
60 persistent-failure clips (epoch 5460): ankle/toe error (0.214m) was ~54%
larger than wrist/hand error (0.139m), both well above the remaining body
groups (0.036-0.117m) -- see note/README.note.md #20 for the full analysis,
theoretical foundation (DeepMimic/H2O/ExBody/ExBody2), and this
implementation plan.

Isolated single-variable test: only `gt_rew`'s body_weights changes here.
Everything else (gr/gv/gav/rh rewards, obs, MoE actor, PD control) is
identical to mlp_moe.py so any difference in outcome is attributable to this
one change.
...
Trained from scratch, no --checkpoint.
    ...
    python protomotions/train_agent.py \\
        ...
        --motion-file <same-motion-file-as-hhi_moe_20946_2shape> \\
...
# Root + 2 wrists + 2 ankles, per note/README.note.md #20's per-joint error
# breakdown. key_weight=3.0 is a first guess, not derived from theory or
# data -- a hyperparameter to sweep once this test's result is in.
KEY_BODIES = ["Pelvis", "L_Wrist", "R_Wrist", "L_Ankle", "R_Ankle"]
KEY_BODY_WEIGHT = 3.0
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    reward_components = {
        ...
        **mimic_tracking_rewards_factory(
            ...
            gav_coef=-0.1,
            rh_coef=-100.0,
            gt_body_weights=build_key_body_weights(
                robot_cfg.kinematic_info,
                key_bodies=KEY_BODIES,
                key_weight=KEY_BODY_WEIGHT,
            ),
        ),
