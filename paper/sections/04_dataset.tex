...
and we keep the two units distinct throughout when reporting dataset size or
evaluation coverage. Figure~\ref{fig:data_pipeline} shows the construction
paths and Figure~\ref{fig:dataset_scale} the resulting scale and split
structure.
...
Appendix~\ref{app:preprocessing} -- a subset of AMASS, not its entirety.
Source SMPL+H sequences are processed at 20\,Hz, including the Z-up to Y-up
coordinate conversion used by the HumanML3D/HUMOS preprocessing pipeline.
Clip extraction, temporal cropping, and mirroring preserve the HumanML3D
identities, and the preparation also includes treadmill/skating cleanup.
...
leaving 20,951 identities for the flat-ground imitation task. This filter is
a task-scope heuristic rather than a feasibility proof: some retained
references may still involve implicit support the filter did not catch. The
retained and excluded ID lists provide the audit trail for that decision.

Source descriptions are available through a separate clip-ID lookup and the
HumanML3D annotation metadata used by HUMOS, associated with the variants of
a source clip rather than newly annotated for each generated body. A coarse
coverage check against a single-caption-per-clip lookup finds a caption for
all 20,951 clip identities, though semantic fidelity is not separately
established. Caption inheritance is imperfect for mirrored clips in
particular: of the 9,579 mirrored identities, 22\% carry a caption
byte-identical to their un-mirrored source, and 40\% contain an explicit
left/right or clockwise term that mirroring inverts, so a mirrored clip's
caption cannot be corrected by simply stripping a mirror prefix. Segment
timing and multi-annotation coverage against the full HumanML3D annotation
set are left for future auditing.
...
body-model variants, giving 128 body configurations rather than 128
independently sampled beta vectors. The actual coefficient arrays and stable
beta keys identify the bodies. Uniform sampling of model coefficients is a
controlled source of variation, not a claim of representing the distribution
...
model~\cite{humos}, using its root-relative 6D root and joint rotation
configuration with shape and body-model conditioning, and retain the
generated pose and root sequences for conversion. HUMOS is an existing
identity-conditioned model; neither its training nor a text-to-motion
generator is a contribution of this work. The checkpoint used in the
documented pipeline is identified in Appendix~\ref{app:provenance}.
...
24 rigid bodies and 69 actuated joint coordinates, each asset carrying its own
collision proxies, joint ranges, and estimated masses/inertias -- simulation-model
estimates rather than measurements of corresponding human participants.
...
artifacts while preserving the generated motion. We therefore assess it with
both artifact measures (sliding, penetration, and jerk) and fidelity measures
(rotation/root changes and retained cross-shape variation), since improved
kinematics alone would not demonstrate improved policy learning or dynamic
feasibility (Section~\ref{sec:refinement_results}).
...
parameters. Captions remain a separately associated metadata resource rather
than a field of every training tensor file. This packaging allows the
training system to stream clip files without downloading the complete corpus
at startup.
...
These splits test held-out clip identities on the trained body set, but they
are not demonstrated to be disjoint by subject or original capture recording,
since different segments can still share a source recording, which limits
claims about broader motion generalization.

\subsection{Current resource scope}

The current resource combines generated references, matching assets, inherited
text associations, and reproducible splits, but is not a fully audited
...
shape-conditioned semantic fidelity are left for future work, and can be
developed independently of the reference-conditioned controller evaluated
here.
