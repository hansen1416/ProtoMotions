...
targets drive the simulator's built-in position PD controller directly,
rather than as a residual added to the reference pose.
...
a body-level mass scaling rather than a single scale based on total character
mass, which does not by itself guarantee dynamically equivalent actuation
across geometrically different bodies. Base gains and limits are listed in
...
heading-frame position feature. Table~\ref{tab:observations} lists the
resulting input dimensions and slot counts.
...
The temporal context is designed to expose both recent changes and more
distant motion information, with the future targets providing a short
anticipation window; we have not separately ablated these design choices.
History is initialized consistently with reference-state resets using
available historical reference states, falling back to single-state
initialization when a trajectory history is unavailable. Contact flags, an
explicit phase scalar, text, and estimated physical feature vectors are not
part of the selected observation.
...
In the selected model, betas enter only through this concatenation: they are
not separately rescaled, embedded as their own attention token, or used for
adaLN modulation.
...
\end{equation}
The weights are unnormalized (they do not sum to one), and this experiment
uses no explicit effort, action-smoothness, or contact-matching reward term,
and no source-dependent switch between canonical and HUMOS reward functions.
...
generalized advantage estimation~\cite{gae} parameter 0.95, policy-ratio
clipping 0.2, and normalized advantages.
...
and a floor of 0.05, with exploration coefficient 1.0 -- a training
curriculum rather than uniform sampling of the complete corpus.

The resident pool is rebuilt every 256 epochs. The full-scale run initially
used 256 clips per rank and was resumed with 128 after memory failures, with
the local cache multiplier reduced to 1.0; these changes affect residency
and memory demand only, not the underlying training split. Validation
membership is fixed independently of pool rebuilding, and validation
evaluation does not update training sampling weights, so we report
resident-pool and validation metrics separately as they measure different
populations.
