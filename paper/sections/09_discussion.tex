...
scale of the constructed corpus. The corrected 150-clip grid selects
slot/type attention without adaLN-Zero, on the basis of a large, replicated
jerk difference rather than the non-discriminating success-rate column, and
this ordering is single-seed. The refinement audit shows the processed
references have substantially less penetration and foot sliding at negligible
fidelity cost, but we stop short of a causal learning benefit from refined
training: the refined and unrefined runs track different targets and were
not cross-evaluated on a common reference.

The physical analysis in Section~\ref{sec:physics} adds a cautionary result
about the choice of spread statistic itself. A coefficient-of-variation
comparison, an otherwise natural way to normalize cross-shape variability by
each clip's own baseline demand, produced a highly significant difference in
exactly the direction opposite the hypothesis, because it divides by a small
number for quasi-static clips. Only a clip-level test on absolute,
unit-preserving residuals revealed this as an artifact rather than a real
effect. We surface this as a methodological point for similar cross-body
comparisons, independent of whether the underlying interaction hypothesis
holds for other checkpoints or corpora.
...
evaluation horizon. The unseen-morphology test in
Section~\ref{sec:unseen_morphology} supports morphology conditioning over
memorization for 28 of 32 held-out shapes, but the remaining 4 are a
genuine, unresolved anomaly, and an explicit all-panel evaluation of the 128
trained shapes remains future work. Any morphology-generalization statement
is confined to body shapes inside the trained coefficient range rather than
arbitrary bodies.
