...
\begin{enumerate}
    ...
    evaluation of the selected checkpoint, a caption-grounded analysis of the
    residual failures, and a generalization test on 32 body shapes excluded
    ...
    of the development corpus to test whether dynamic motions are more
    shape-sensitive than quasi-static ones, rather than simply assuming it.
\end{enumerate}
...
Section~\ref{sec:unseen_morphology} extends the claim to bodies excluded
from training entirely: 32 new shapes drawn from the trained coefficient
range, of which 28 track successfully at the trained-body level with no
decline across the evaluated distance from training, while 4 fail severely
in a pattern pointing to an asset artifact rather than a genuine shape
effect. Section~\ref{sec:physics} tests the morphology$\times$motion-class
