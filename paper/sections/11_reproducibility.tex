...
\begin{itemize}
    ...
    \item Selected full-scale experiment (\path{examples/experiments/mimic/}):
    \path{mlp_wide_stage2_discover_attention_slot_type.py},
    ...
    \item Physical analysis (Section~\ref{sec:physics}): semantic
    classification in \path{tools/classify_clip_semantics.py}, the
    instrumented replay in \path{tools/analyze_morphology_motion_interaction.py},
    and the clip-level significance test in
    \path{tools/test_morphology_motion_variance_significance.py}.
    \item Unseen-morphology generalization (Section~\ref{sec:unseen_morphology}):
    held-out shape assets in \path{protomotions/data/assets/mjcf/smpl_mor_interp/}
    (generation pipeline in \path{note/README.heldout-pipeline.md}), the
    replay in \path{tools/analyze_unseen_morphology_generalization.py}, and
    the distance/split join in
    \path{tools/aggregate_unseen_morphology_generalization.py}.
\end{itemize}
...
Two items remain before a finalized benchmark report: archiving the
immutable HUMOS/SMPLSim checksums alongside the dataset release, and
releasing the retained per-motion evaluation records for the validation and
test splits.
