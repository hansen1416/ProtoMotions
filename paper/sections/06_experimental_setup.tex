...
Status of reported evidence & Completed, Table~\ref{tab:ablation_results} & Epoch-28,170 checkpoint, Table~\ref{tab:test_results} \\
\bottomrule
\end{tabularx}
\end{table}
...
The corrected comparison is a four-architecture by two-reference-version grid,
Table~\ref{tab:ablations}. The four architectures are a wide flat temporal
MLP, basic temporal attention, attention with slot/type embeddings, and that
last model plus actor-only adaLN-Zero. Each is trained once on refined
references (A--D) and once on unrefined references (E--H); slot/type on
unrefined references is labelled E rather than a separate letter for
historical reasons. All eight runs use one seed, a single A40, the same
6,000-epoch budget, evaluation cadence, and shape-sampling seed.

\begin{table}[t]
\centering
\small
\caption{Corrected 150-clip grid: four architectures $\times$ two reference
versions, one seed each. Results are in Table~\ref{tab:ablation_results}.}\label{tab:ablations}
...
All eight runs use seed 0 and one A40 GPU per policy. Morphology
concatenation into both heads is retained in D and H; adaLN-Zero is additional
actor conditioning, not a replacement for the critic or a two-stage schedule.

The models do not have equal parameter counts (Table~\ref{tab:compute_accounting}).
The grid is single-seed; the architecture ordering in
Table~\ref{tab:ablation_results} rests on the jerk difference, which recurs
under both reference versions, and not on the success-rate column, which is
treated as non-discriminating. The refined and unrefined pairs (A/F, B/G, C/E, D/H) are each
evaluated against their own reference version, so they measure whether refined
training costs policy quality, not whether it causally improves accuracy; the
refinement argument is completed by the offline reference-artifact audit in
Section~\ref{sec:refinement_results} rather than by a common-reference
controller re-evaluation. A causal comparison would require scoring
refined- and unrefined-trained checkpoints on one shared reference version.
...
error within each frame -- the logged \emph{mean maximum-joint error}, a
per-frame worst-body statistic averaged over time rather than a single
dataset-wide maximum. Global body-rotation error is a whole-body geodesic
measure, not a local joint-angle/DOF error.
...
clips on unseen bodies are different evaluations. The main validation and
test tables address only the first. A single rotating panel does not
measure all 128 variants of every clip, and future cross-shape comparisons
must report the actual body identities and number of evaluated pairs. The
unseen-morphology evaluation in Section~\ref{sec:unseen_morphology} covers
the other two axes directly, on shapes drawn from the trained coefficient
range but excluded from the 128 training configurations, with results
stratified by distance to the nearest trained morphology.
...
morphology encoder), consistent with, but not sufficient by itself to
explain, the jerk difference in Section~\ref{sec:architecture_results}.

The full-scale selected run shares C and E's architecture exactly (same
encoder, transformer, and head widths --
\path{mlp_wide_stage2_discover_attention_slot_type.py} inherits these
constants unchanged from the 150-clip experiment file), so its parameter
count is also 51,238,582. It ran on six A40 GPUs (\texttt{ddp}, 32-bit,
2,048 environments and an 8,192 minibatch per GPU) for roughly 28,170
epochs. The logged rank-0 wall time is about 77\,h, of which one interruption
near epoch 760 accounts for about 3\,h, giving on the order of 450 GPU-hours;
this run is not directly comparable to the eight matched single-GPU
development runs. An early restart after a memory-related failure resumed
from the last checkpoint with the resident pool reduced from 256 to 128
clips per rank and the local cache multiplier set to 1.0; because configs
are reloaded from the pickle on resume, the frozen configuration still
records the pre-restart pool size of 256, and the earlier failed launch,
logged into the same run, is not separately quantifiable pending recovery of
the launcher logs. We draw no compute-efficiency claim from this accounting:
the temporal MLP's lower wall-clock cost in Table~\ref{tab:compute_accounting}
is not weighed here against its higher position error relative to slot/type
in Table~\ref{tab:ablation_results}.

At the rented rate for this hardware (\$0.49/GPU-hour for the A40, plus
\$0.014/hour for the 100\,GB volume used by the single-GPU development runs
and \$0.044/hour for the 320\,GB volume used by the six-GPU full-scale run),
the eight-run ablation grid and the full-scale training run together cost
approximately \$315: about \$85 for the ablation grid and \$230 for the
full-scale run. This reflects one vendor's rate at the time of training, not
a general claim about training cost.
